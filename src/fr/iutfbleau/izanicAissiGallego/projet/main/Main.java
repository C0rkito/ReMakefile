...
import fr.iutfbleau.izanicAissiGallego.projet.cible.*;
...
public class Main {
	...
	*/
    ...
    public static void main(String[] args) {
		...
		Fichier bakefile = new Fichier("bakefile");
		
		if (!bakefile.existe()) {
			System.out.println("bake: *** No targets specified and no makefile found.  Stop.");
			System.exit(0);
		}
        ...
        for (String cible : cibleAFaire) {
			...
			if(file.size() == 1 && file.peekFirst().estPhony() && file.peekFirst().getRecettes().size() == 1 && file.peekFirst().getRecettes().get(0).equals("")){
